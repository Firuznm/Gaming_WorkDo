import { createContext, useState} from "react"
    ...
    const addToCart = (product) => {
        setState({
            ...state,
 basket: state.basket.find((Item) => Item.id === product.id) ? state.basket.map((Item) => Item.id === product.id
    ...
    //  basketde atilan yeni basketin icinde olan praductun sayina artirma funksiyasi
    const increase = (id) => {
        setState({
            ...state,
    ...
    const totalPraduct = state.basket.reduce(
        (total, praduct) =>
            (total = total + praduct.price * praduct.count),
        0
    );
	return(
